# OWS Programmable Policy — 設計ドキュメント

## 1. プロジェクト概要

**OWS Programmable Policy** は、Open Wallet Standard の Policy Engine を拡張し、開発者が TypeScript でカスタムポリシーを書ける SDK + ランタイム。
...
2. Programmable Policy Runtime がポリシーを順次評価
...
Agent → OWS sign → Programmable Policy
                        ...
                        └── 高リスク検出時:
                            ...
                            │
                            ├── 4. Programmable Policy が承認トークンを検証
                            │      → 有効 → Allow
                            │      → 無効/期限切れ → Deny
                            │
                            └── 5. タイムアウト（例: 30分）
